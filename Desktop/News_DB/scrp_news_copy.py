...
def save_files(content_dict, basepath, str_now):
    str_now2 = re.sub(r'(-|:)','_',str_now)
    str_now3 = re.sub(r'\.[0-9]+$','',str_now2)
    str_now = str_now3
    print(str_now)
    #save csv file as $date.csv
    csv_file_name = basepath + '/Data/'  + keyword + '/' + str_now + '_' + keyword + '.csv'
    with open(csv_file_name, 'w') as f:
        for key in content_dict.keys():
            f.write("%s,%s\n"%(key,content_dict[key]))
        f.close()
    tts=gTTS(text=str_full_text, lang='zh', slow=False)
    radio_filename = basepath + '/Data/' + keyword + '/' + str_now + '_' + keyword + '.mp3'
...
basepath='/Users/rubylintu/Desktop/News_DB/'
content_dict={}
keyword = sys.argv[1]
print(keyword)
#initialize
...
return_val = save_files(content_dict, basepath, str_now)
print(return_val)
