...
def _run_intraday_model(stock_df):
    """
    用 IntradayParticleModel 跑整天 tick，產生動態預測曲線

    Args:
        stock_df: 單一股票的 intraday DataFrame

    Returns:
        list of (timestamp, predicted_price) 或 None
    """
    try:
        ...
        for _, row in stock_df.iterrows():
            model.update(row['price'], row.get('trade_vol', 0),
                         row.get('timestamp'))

        curve = model.get_prediction_curve()
        if len(curve) > 5:
            return curve
...
def plot_stock_page(ax_price, ax_vol, stock_df, code, name,
    ...
    Args:
        ...
        prediction: 粒子模型盤前預測 dict (optional, legacy)
    """
    if stock_df.empty:
        return
    ...
    # === 粒子模型動態預測曲線 ===
    pred_curve = _run_intraday_model(stock_df)
    if pred_curve:
        pred_dts = []
        pred_prices_curve = []
        for ts, pp in pred_curve:
            dt = parse_timestamp(ts)
            if dt:
                pred_dts.append(dt)
                pred_prices_curve.append(pp)

        if len(pred_dts) > 5:
            ax_price.plot(pred_dts, pred_prices_curve,
                          color='#E74C3C', linewidth=1.0, alpha=0.75,
                          linestyle='-', zorder=2, label='Particle')

            # 計算 MAE
            min_len = min(len(prices), len(pred_prices_curve))
            mae = np.mean(np.abs(
                np.array(prices[:min_len]) -
                np.array(pred_prices_curve[:min_len])
            ))
            avg_p = np.mean(prices)
            mae_pct = mae / avg_p * 100 if avg_p > 0 else 0

            ax_price.text(dts[-1], pred_prices_curve[-1],
                          f'  MAE {mae_pct:.2f}%',
                          fontproperties=_CJK_FONT_SMALL,
                          color='#E74C3C', verticalalignment='bottom')
...
def _draw_legend_page(pdf, date_fmt, num_stocks, num_gpt, num_gem):
    ...
    items = [
        ...
        ('line',    '#E74C3C', '━━',  '粒子模型預測曲線',  'tick-by-tick 動態預測，紅線為模型預測走勢'),
