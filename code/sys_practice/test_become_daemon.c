...
{  
    ...
    /* 屏蔽一些有關控制終端操作的信号 
     * 防止在守護程序沒有正常運轉起來時，因控制終端受到幹擾退出或挂起 
     * */  
    signal(SIGINT,  SIG_IGN);// 終端中斷  
    signal(SIGHUP,  SIG_IGN);// 連接配接挂斷  
    signal(SIGQUIT, SIG_IGN);// 終端退出  
    signal(SIGPIPE, SIG_IGN);// 向無讀程序的管道寫資料  
    signal(SIGTTOU, SIG_IGN);// 背景程式嘗試寫操作  
    signal(SIGTTIN, SIG_IGN);// 背景程式嘗試讀操作  
